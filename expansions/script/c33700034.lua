...
function c33700034.filter(c,e,tp)
	return (c:GetSequence()==6 or c:GetSequence()==7) and c:IsSetCard(0x3440) and c:IsCanBeSpecialSummoned(e,0,tp,false,false)
...
function c33700034.target(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	if chkc then return chkc:IsLocation(LOCATION_SZONE) and chkc:IsControler(tp) and c33700034.filter(chkc,e,tp) end
	if chk==0 then return Duel.GetLocationCount(tp,LOCATION_MZONE)>0 and
		Duel.GetLocationCount(tp,LOCATION_SZONE)>0
		and Duel.IsExistingTarget(c33700034.filter,tp,LOCATION_SZONE,0,1,nil,e,tp) end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_SPSUMMON)
	local g=Duel.SelectTarget(tp,c33700034.filter,tp,LOCATION_SZONE,0,1,1,nil,e,tp)
